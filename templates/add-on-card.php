        ...
        <div class="action-links">
            <ul class="plugin-action-buttons">
				<?php if ( isset( $rcl_addons[ $addon->slug ] ) && version_compare( $addon->version, $rcl_addons[ $addon->slug ]['version'] ) > 0 ): ?>
                    <li><a class="update-now button aria-button-if-js" data-addon="<?php echo $addon->slug; ?>" href="#"
				...
				<?php elseif ( isset( $rcl_addons[ $addon->slug ] ) ): ?>
                    <li><span class="button button-disabled"
                              title="<?php _e( 'This add-on has already been installed', 'wp-recall' ) ?>"><?php _e( 'Installed', 'wp-recall' ) ?></span>
                    </li>
				<?php else: ?>
                    <li><a class="button" target="_blank" data-slug="<?php echo $addon->slug; ?>"
                           href="<?php echo $addon->add_on_uri; ?>"
                           aria-label="<?php _e( 'Go to page', 'wp-recall' ) ?> <?php echo $addon->name; ?> <?php echo $addon->version; ?>"
                           data-name="<?php echo $addon->name; ?> <?php echo $addon->version; ?>"><?php _e( 'Go to', 'wp-recall' ) ?></a>
                    </li>
				<?php endif; ?>
                <li><a href="#" class="open-addon-details-modal"
                       ...
                       aria-label="Подробности о <?php echo $addon->name; ?>" data-title="<?php echo $addon->name; ?>">Детали</a>
                </li>
        </div>
        <div class="desc column-description">
            <p><?php print_r( $addon->description ); ?></p>
            <p class="authors"><cite><?php _e( 'Author', 'wp-recall' ) ?>: <a href="<?php echo $addon->author_uri; ?>"
                                                                              target="_blank"><?php echo $addon->author; ?></a></cite>
            </p>
        </div>
        <div class="addon-terms">
			...
			if ( isset( $addon->terms ) && $addon->terms ):
				...
				foreach ( $addon->terms as $taxonomy => $terms ) {
                    ...
                    <p>
                        <cite><?php echo ( $taxonomy == 'prodcat' ) ? __( 'Category', 'wp-recall' ) : __( 'Tags', 'wp-recall' ) ?>
                            :
